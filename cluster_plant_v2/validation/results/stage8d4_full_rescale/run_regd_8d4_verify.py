"""Stage 8D4 RegD verification: baseline vs SOC-aware NMPC after full rescale.

Trimmed rerun of the RegD closed loop under the Stage 8D4 chiller
(displacement 72 cc/rev, evaporator 5.5 m2, condenser 8 m2, air 6.5 kg/s):
fixed-speed baseline plus the SOC-aware NMPC only, to verify the rescaled
capacity holds the 25 +/- 0.65 K band through the 1C-charge spikes that the
8D3 chiller could not cover (out-of-band 1325 s there).
"""
...
for name, frame in frames.items():
    ...
    frame.to_csv(HERE / f"case_T2_regd_8d4_{name}_timeseries.csv", index=False)

summary = pd.DataFrame(rows)
summary["scenario"] = "T2_regd_6400s_8d4"
summary.to_csv(HERE / "regd_8d4_summary.csv", index=False)
