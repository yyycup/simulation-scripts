...
print("pass 2: SOC-aware NMPC with compressor thermostat cycling ...", flush=True)
socaware = run_nmpc(
    case,
    duration_s=DURATION,
    dt_s=DT,
...
for name, frame in frames.items():
    ...
    frame.to_csv(HERE / f"case_S0_peak_8d4e2_{name}_timeseries.csv", index=False)
...
summary["scenario"] = "S0_peak_6400s_8d4e2"
summary.to_csv(HERE / "s0_peak_8d4e2_summary.csv", index=False)
