"""Stage 8D4f cluster-calibration before/after comparison figure.

Two rows (8D4e single-pack-calibrated predictor vs 8D4f cluster-corrected
predictor), two columns (temperature with control band plus 10-min block
means, compressor command). Shows the residual steady drift of 8D4e is
removed by the 8D4f plate-flow/return correction. Style per project spec:
...
runs = {
    "8D4e（单包口径预测器，残余漂移）":
        "case_S0_peak_8d4e_nmpc_soc_aware_8d4d_timeseries_pre8d4f.csv",
    "8D4f（簇口径修正 + 重锚定）":
...
for row, (label, fname) in enumerate(runs.items()):
    ...
    ax.set_title(
        ...
        f"（偏差 {late.mean() - REF_C:+.2f} K），带外 {out_band:.0f} s",
        fontsize=10,
    )
...
fig.suptitle(
    "8D4 调峰工况（S0, 560 A 恒流, 6400 s）：8D4f 簇口径修正前后对比",
    fontsize=13,
)
fig.tight_layout(rect=[0, 0, 1, 0.97])
fig.savefig(RES / "fig_s0_peak_8d4f_before_after.png", dpi=150)
fig.savefig(RES / "fig_s0_peak_8d4f_before_after.pdf")
print("saved to", RES)
