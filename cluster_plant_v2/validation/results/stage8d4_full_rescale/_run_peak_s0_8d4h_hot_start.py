"""Stage 8D4h peak-shaving NMPC pass with a hot-start plant (S0, 560 A).

The 8D4g run revealed a start-up dip: build_final_plant() calls
ClusterPlant.from_equilibrium() at the baseline 4000 rpm, so the loop
begins pre-chilled (supply ~19 C) with a fully charged evaporator
(~17.6 kW burst at t=5 s) and the battery drops to 24.76 before
recovering. Per the user's directive the plant must start hot-biased,
not cold-biased.

This variant builds the plant locally with:

- battery / cold plate / tank initial temperature HOT_START_TEMP_C
  (27.0 C, 2 K above the 25 C setpoint);
- compressor actuator initialized at the 1000 rpm floor (the cycle
  solver rejects rpm < 1000), so the evaporator starts near its
  minimum charge instead of the 4000 rpm equilibrium;
- initial compressor command at the floor instead of the 4000 rpm
  baseline, so the NMPC ramps capacity up as needed.

No thermostat cycling overlay (same as 8D4g), only the band-floor
safety net. Output names are distinct so 8D4g results stay intact.
"""
...
# Hot-start initial condition: battery/plate/tank 2 K above setpoint.
...
CKPT_CSV = HERE / "_8d4h_nmpc_ckpt.csv"
BASELINE_SRC = HERE / "case_S0_peak_8d4d_baseline_fixed_timeseries.csv"
NMPC_CSV = HERE / "case_S0_peak_8d4h_nmpc_hot_start_timeseries.csv"
BASELINE_DST = HERE / "case_S0_peak_8d4h_baseline_fixed_timeseries.csv"
SUMMARY_CSV = HERE / "s0_peak_8d4h_summary.csv"
...
def build_hot_start_plant() -> ClusterPlant:
    """Same topology as build_final_plant but with a hot, un-chilled start."""
...
for step_index in range(start_step, STEPS):
    ...
    rows.append(
        dict(
            controller="physics_p_nmpc_8d4h_hot_start",
...
for name, frame in {
    ...
    "nmpc_8d4h_hot_start": nmpc.sort_values("time_s"),
}.items():
...
summary["scenario"] = "S0_peak_6400s_8d4h"
