"""Stage 8D4g peak-shaving NMPC pass without the thermostat cycling overlay.

8D4f (cluster-calibrated predictor) tracks 25 C with zero steady offset,
but the 8D4d compressor start/stop overlay (24.85/25.15 hysteresis)
still bangs the machine on/off for the first ~60 min, producing a
~0.5 K, ~7 min temperature limit cycle. The overlay is not physically
necessary here: the 1000 rpm floor supplies ~4.6 kW while the early
load is ~5.5 kW, so continuous low-speed modulation can match the load
(the run itself settles into continuous 1095~3106 rpm after 3640 s).

This variant therefore drops the thermostat overlay entirely and keeps
only a band-floor safety net: force the compressor off only if the
battery mean temperature falls below SAFETY_OFF_TEMP_C (well under the
25 +/- 0.65 band floor), and restore it above SAFETY_ON_TEMP_C. The
safety net is expected never to trigger in this scenario; its firing
count is logged.

Same checkpoint/progress machinery and output conventions as
_run_peak_s0_8d4e_nmpc_only.py, with distinct file names so the 8D4f
results stay intact.
"""
...
CKPT_CSV = HERE / "_8d4g_nmpc_ckpt.csv"
BASELINE_SRC = HERE / "case_S0_peak_8d4d_baseline_fixed_timeseries.csv"
NMPC_CSV = HERE / "case_S0_peak_8d4g_nmpc_no_cycling_timeseries.csv"
BASELINE_DST = HERE / "case_S0_peak_8d4g_baseline_fixed_timeseries.csv"
SUMMARY_CSV = HERE / "s0_peak_8d4g_summary.csv"
...
for step_index in range(start_step, STEPS):
    ...
    rows.append(
        dict(
            controller="physics_p_nmpc_8d4g_no_cycling",
...
for name, frame in {
    ...
    "nmpc_8d4g_no_cycling": nmpc.sort_values("time_s"),
}.items():
...
summary["scenario"] = "S0_peak_6400s_8d4g"
