...
def create_controller(
    ...
    if control == "mpc":
        from .gekko.mpc import create_mpc_flow_controller
    ...
    if control == "state_space_qp":
        from .fixed_qp.adapter import StateSpaceQPPlantController

        return StateSpaceQPPlantController(
            current_profile,
            dt=dt,
            target_temp_c=target_temp_c,
            case_name=case_name,
            predictor_artifact=mpc_predictor_artifact,
            horizon_override=mpc_horizon_override,
            qp_weights=state_space_qp_weights,
            linear_model_bank_path=state_space_qp_linear_model_bank,
        )
    if control == "physics_p_dompc":
        from .dompc.adapter import PhysicsPDoMPCPlantController
        return PhysicsPDoMPCPlantController(
            current_profile, dt=dt, target_temp_c=target_temp_c,
            case_name=case_name, predictor_artifact=mpc_predictor_artifact,
            control_interval_steps=dompc_control_interval_steps,
            control_horizon=dompc_control_horizon,
        )
