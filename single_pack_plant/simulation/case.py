...
``simulate_case()`` 在每个 5 s 时间步读取工况、请求控制器指令、推进完整
Plant 并写入结果。NMPC 的控制器入口由 ``controllers/factory.py`` 选择。
"""
...
from ..plant.age_model import AgingModel280Ah
from ..predictor.shadow import PhysicsPShadowPredictor, battery_heat_generation_w
...
from ..plant.thermal_loop import (
...
OUTPUT_COLUMN_RENAMES = {
    ...
    "MPC_Objective_Contributions_Valid": "MPC objective contributions valid",
    "MPC_Objective_Contributions_Error": "MPC objective contributions error",
    "MPC_Objective_Solver_Total": "MPC objective solver total",
    "MPC_Objective_CV_Contribution": "MPC objective CV contribution",
    "MPC_Objective_Temperature_Contribution": "MPC objective temperature contribution",
    "MPC_Objective_Terminal_Contribution": "MPC objective terminal contribution",
    "MPC_Objective_Comp_Contribution": "MPC objective compressor contribution",
    "MPC_Objective_Pump_Contribution": "MPC objective pump contribution",
    "MPC_Objective_DComp_Contribution": "MPC objective compressor move contribution",
    "MPC_Objective_DPump_Contribution": "MPC objective pump move contribution",
    "MPC_Objective_Other_Contribution": "MPC objective other contribution",
    "MPC_Objective_CV_Share": "MPC objective CV share",
    "MPC_Objective_Temperature_Share": "MPC objective temperature share",
    "MPC_Objective_Terminal_Share": "MPC objective terminal share",
    "MPC_Objective_Comp_Share": "MPC objective compressor share",
    "MPC_Objective_Pump_Share": "MPC objective pump share",
    "MPC_Objective_DComp_Share": "MPC objective compressor move share",
    "MPC_Objective_DPump_Share": "MPC objective pump move share",
    "MPC_Objective_Other_Share": "MPC objective other share",
    "MPC_Objective_CV_Raw": "MPC objective CV raw",
    "MPC_Objective_CV_Scale": "MPC objective CV scale",
    "MPC_Objective_CV_Normalized": "MPC objective CV normalized",
    "MPC_Objective_Temperature_Raw": "MPC objective temperature raw",
    "MPC_Objective_Temperature_Scale": "MPC objective temperature scale",
    "MPC_Objective_Temperature_Normalized": "MPC objective temperature normalized",
    "MPC_Objective_Terminal_Raw": "MPC objective terminal raw",
    "MPC_Objective_Terminal_Scale": "MPC objective terminal scale",
    "MPC_Objective_Terminal_Normalized": "MPC objective terminal normalized",
    "MPC_Objective_Compressor_Raw": "MPC objective compressor raw",
    "MPC_Objective_Compressor_Scale": "MPC objective compressor scale",
    "MPC_Objective_Compressor_Normalized": "MPC objective compressor normalized",
    "MPC_Objective_Pump_Raw": "MPC objective pump raw",
    "MPC_Objective_Pump_Scale": "MPC objective pump scale",
    "MPC_Objective_Pump_Normalized": "MPC objective pump normalized",
    "MPC_Objective_DComp_Raw": "MPC objective compressor move raw",
    "MPC_Objective_DComp_Scale": "MPC objective compressor move scale",
    "MPC_Objective_DComp_Normalized": "MPC objective compressor move normalized",
    "MPC_Objective_DPump_Raw": "MPC objective pump move raw",
    "MPC_Objective_DPump_Scale": "MPC objective pump move scale",
    "MPC_Objective_DPump_Normalized": "MPC objective pump move normalized",
...
def simulate_case(
    ...
    *,
    ...
    state_space_qp_weights=None,
    state_space_qp_linear_model_bank=None,
    dompc_control_interval_steps=None,
    dompc_control_horizon=None,
    mpc_runtime_weight_multipliers=None,
    mpc_runtime_objective_multipliers=None,
    mpc_runtime_uniform_objective_scale=None,
    mpc_objective_normalization_scales=None,
    mpc_objective_diagnostics=False,
    ...
    controller = create_controller(
        ...
        state_space_qp_weights=state_space_qp_weights,
        state_space_qp_linear_model_bank=state_space_qp_linear_model_bank,
        dompc_control_interval_steps=dompc_control_interval_steps,
        dompc_control_horizon=dompc_control_horizon,
    )
    if mpc_runtime_weight_multipliers is not None:
        if mpc_runtime_objective_multipliers is not None:
            raise ValueError(
                "legacy and explicit MPC runtime multipliers are mutually exclusive"
            )
        if control not in {"mpc", "state_space_qp"} or not hasattr(
            controller, "set_runtime_weight_multipliers"
        ):
            raise ValueError(
                "mpc_runtime_weight_multipliers requires a compatible MPC controller"
            )
        multipliers = tuple(mpc_runtime_weight_multipliers)
        if len(multipliers) != 3:
            raise ValueError(
                "mpc_runtime_weight_multipliers must contain exactly three values"
            )
        controller.set_runtime_weight_multipliers(*multipliers)
    if mpc_runtime_objective_multipliers is not None:
        if control != "mpc" or not hasattr(
            controller, "set_runtime_objective_multipliers"
        ):
            raise ValueError(
                "mpc_runtime_objective_multipliers requires an MPC controller"
            )
        multipliers = tuple(mpc_runtime_objective_multipliers)
        if len(multipliers) != 2:
            raise ValueError(
                "mpc_runtime_objective_multipliers must contain exactly two values"
            )
        controller.set_runtime_objective_multipliers(*multipliers)
    if mpc_runtime_uniform_objective_scale is not None:
        if control != "mpc" or not hasattr(
            controller, "set_runtime_uniform_objective_scale"
        ):
            raise ValueError(
                "mpc_runtime_uniform_objective_scale requires an MPC controller"
            )
        controller.set_runtime_uniform_objective_scale(
            mpc_runtime_uniform_objective_scale
        )
    if mpc_objective_normalization_scales is not None:
        if control != "mpc" or not hasattr(
            controller, "set_runtime_objective_normalization"
        ):
            raise ValueError(
                "mpc_objective_normalization_scales requires an MPC controller"
            )
        controller.set_runtime_objective_normalization(
            mpc_objective_normalization_scales
        )
    if mpc_objective_diagnostics:
        if control != "mpc" or not hasattr(
            controller, "set_objective_contribution_diagnostics_enabled"
        ):
            raise ValueError("mpc_objective_diagnostics requires an MPC controller")
        controller.set_objective_contribution_diagnostics_enabled(True)
    ...
    if mpc_params is not None or getattr(controller, "uses_dynamic_refrigeration", False):
    ...
    for step_no, _row in enumerate(old_df.itertuples(index=False)):
        ...
        if flow_info:
            ...
            record["MPC_Alpha_Temp"] = flow_info.get("alpha_temp", np.nan)
            record["MPC_Alpha_Comp"] = flow_info.get("alpha_comp", np.nan)
            record["MPC_Alpha_Pump"] = flow_info.get("alpha_pump", np.nan)
            record["MPC_Runtime_WSPLO"] = flow_info.get("runtime_WSPLO", np.nan)
            record["MPC_Runtime_WSPHI"] = flow_info.get("runtime_WSPHI", np.nan)
            record["MPC_Runtime_W_Terminal"] = flow_info.get(
                "runtime_w_terminal", np.nan
            )
            record["MPC_Runtime_W_Energy_Comp"] = flow_info.get(
                "runtime_w_energy_comp", np.nan
            )
            record["MPC_Runtime_W_Energy_Pump"] = flow_info.get(
                "runtime_w_energy_pump", np.nan
            )
            record["MPC_Runtime_Objective_Scale_Factor"] = flow_info.get(
                "runtime_objective_scale_factor", 1.0
            )
            record["MPC_Runtime_W_DComp"] = flow_info.get(
                "runtime_w_dcomp", np.nan
            )
            record["MPC_Runtime_W_DPump"] = flow_info.get(
                "runtime_w_dpump", np.nan
            )
            if mpc_runtime_objective_multipliers is not None:
                record["MPC_Runtime_Weight_Mode"] = flow_info.get(
                    "runtime_weight_mode", ""
                )
                record["MPC_Runtime_W_Temp_Obj"] = flow_info.get(
                    "runtime_w_temp_obj", np.nan
                )
            if mpc_objective_diagnostics:
                record["MPC_Objective_Contributions_Valid"] = bool(
                    flow_info.get("objective_contributions_valid", False)
                )
                record["MPC_Objective_Contributions_Error"] = flow_info.get(
                    "objective_contributions_error", ""
                )
                record["MPC_Objective_Solver_Total"] = flow_info.get(
                ...
                for column_label, diagnostic_key in contribution_columns.items():
                    record[f"MPC_Objective_{column_label}_Contribution"] = flow_info.get(
                        f"objective_{diagnostic_key}_contribution", np.nan
                    )
                    record[f"MPC_Objective_{column_label}_Share"] = flow_info.get(
                        f"objective_{diagnostic_key}_contribution_share", np.nan
                    )
                raw_term_columns = {
                    "CV": "cv",
                    "Temperature": "temperature",
                ...
                for column_label, diagnostic_key in raw_term_columns.items():
                    for quantity in ("raw", "scale", "normalized"):
                        record[
                            f"MPC_Objective_{column_label}_{quantity.title()}"
                        ] = flow_info.get(
                            f"objective_{diagnostic_key}_{quantity}", np.nan
                        )
