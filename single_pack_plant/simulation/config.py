...
T_DIFF_LIMIT_C = 0.5
COMPRESSOR_MAP_MIN_RPM = 2000.0
COMPRESSOR_MIN_STEADY_RPM = 1000.0
N_COMP_MIN_RPM = COMPRESSOR_MIN_STEADY_RPM
PID_N_COMP_MIN_RPM = COMPRESSOR_MIN_STEADY_RPM
...
def initial_soc_for(scene):
    """Return the configured initial SOC for Chinese and runner scene aliases."""
    return INITIAL_SOC_REG if is_frequency_scene(scene) else INITIAL_SOC_PEAK


def is_frequency_scene(scene):
    return str(scene).strip().lower() in {"调频", "freq", "frequency", "reg"}
